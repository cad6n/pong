...
simulate_game(Input* input, float dt) {
	...
	draw_rect(0, 0, arena_half_size_x, arena_half_size_y, 0x5f7fff);
...
#else
	if (ball_p_y > player_1_p + 2.f) player_1_ddp += 1400;
	if (ball_p_y < player_1_p + 2.f) player_1_ddp -= 1400;
#endif
	...
	// Simulate Ball
	{
		...
		if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 80, player_1_p, player_half_size_x, player_half_size_y))
		{
			...
			ball_dp_x *= -1;
			ball_dp_y = (ball_p_y - player_1_p) * 2 + player_1_dp * 0.68f;
			playBeepAsync(200, 350);
		}
		else 	if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, -80, player_2_p, player_half_size_x, player_half_size_y))
		{
			...
			ball_p_x = -80 + player_half_size_x + ball_half_size;
			ball_dp_x *= -1;
			ball_dp_y = (ball_p_y - player_2_p) * 2 + player_2_dp * 0.68f;
			playBeepAsync(200, 350);
		}
